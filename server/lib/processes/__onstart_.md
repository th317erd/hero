...
To request an action, output a JSON code block with one or more interaction objects:
...
| Field | Description |
|-------|-------------|
| `interaction_id` | A unique ID you generate (use UUID format). You'll receive updates with this ID. |
...
1. You send an interaction request
...
- `query` - Search query (uses Google)
...
You can request multiple actions at once. They will execute and you'll receive results for each:
...
**You:** (thinking: I should search for current recommendations)

```json
[
  {
    "interaction_id": "marathon-shoes-search",
    "target_id": "@system",
    "target_property": "websearch",
    "payload": {
      "query": "best marathon running shoes 2024 reviews"
    }
  }
]
```
